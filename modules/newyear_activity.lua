...
if not newyear_lib then
    newyear_lib = _S
    {
        ...
        activ1_statime = "2012-01-17 00:00:00",  --活动开始时间
    	activ1_endtime = "2012-02-08 00:00:00",  --活动结束时间
...
--游戏结束采集盘数
newyear_lib.ongameover = function(user_info,addgold)
--活动一：年兽FUN礼
--[[
基本流程：
...
6、鼠标移到武器上方，显示武器的TIPS,包括名称、描述、奖品、伤害
   ...
   --每天玩够40盘得到烟花，德州VIP1-3 获得烟花*2，德州VIP4-5 获得烟花*3
	...
	--记录到数据库
    local sqltemplet = "update t_attack_monster_rank set play_count = %d, gun1_attack_num = %d, gun2_attack_num = %d, gun3_attack_num = %d, sys_time = now() where user_id = %d;commit;";
    local sql=string.format(sqltemplet, play_count, gun1_value,gun2_value,gun3_value,user_info.userId);
    dblib.execute(sql);
	
end

--定时刷新事件
newyear_lib.ontimecheck = function()
	                ...
	                if(prizeid <= 0) then
		 			...
		 			if(attack_id == 1)then
			            ...
			            elseif(prizeid == 6)then	--5万筹码
							...
							local msg1 = tex_lan.get_msg(user_info, "newyear_activity_msg_awards"); 
			        ...
			        elseif(attack_id == 2)then
			            ...
			            elseif(prizeid == 2)then	--1W筹码
			            	...
			            	--加1W筹码
			  				...
			  				local user_nick=user_info.nick
							user_nick=_tosqlstr(user_nick).."   "
							local msg = tex_lan.get_msg(user_info, "newyear_activity_msg_awards_1")..user_info.nick..tex_lan.get_msg(user_info, "newyear_activity_msg_awards");
							local msg1 = tex_lan.get_msg(user_info, "newyear_activity_msg_awards"); 
							msg1 = string.format(msg1,5); 
							BroadcastMsg(_U(msg)..user_nick.._U(msg1),0)
...
4、排名10分钟更新1次
	...
	--TraceError("-->>>>初始化排行榜")
	newyear_lib.attack_ph_list = {}; --攻击年兽排名
...
function newyear_lib.on_recv_activation(buf)
	...
	my_mc,my_attack_value = newyear_lib.get_my_pm(attack_paimin_list,user_info)
	if(user_info.newyear_attack_value ~= nil)then
		my_attack_value = user_info.newyear_attack_value
	end
	local libao_sign = user_info.newyear_libao_sign		----是否领取了“驱魔礼包”标记
	newyear_lib.send_ph_list(user_info, libao_sign, my_attack_value, my_mc, attack_paimin_list)
