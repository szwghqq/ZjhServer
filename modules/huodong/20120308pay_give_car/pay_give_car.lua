...
function pay_give_car.bag_open_box_event(e)
  ...
  if pay_give_car.already_get_car[box_id] and 
  pay_give_car.already_get_car[box_id][item_gift_id] then
    pay_give_car.already_get_car[box_id][item_gift_id][1] = pay_give_car.already_get_car[box_id][item_gift_id][1] + item_number
    pay_give_car.set_already_get_car_num(box_id, item_gift_id, item_number)
  end
 
  --更新所有打开面板人的开出车的数据
  for k,v in pairs (pay_give_car.user_list) do
    if v.open_pay_give_car_panel == 1 then
      local user_info = usermgr.GetUserById(k)
      pay_give_car.send_huodong_info(user_info)
    end
  end
  local user_info = usermgr.GetUserById(user_id)
  local msg = "使用%s，开出%s。恭喜他，快快参加爱车嘉年华，下一个中奖的就是您哦！"		
...
---数据库操作-------------------
function pay_give_car.set_already_get_car_num(box_id, item_gift_id, add_item_num)
  local num = 0
  for k,v in pairs (pay_give_car.already_get_car[box_id]) do
    if k == item_gift_id then
      num = v.key
    end
  end
  if box_id == 200017 then 
    num = num + 5
  elseif box_id == 200018 then 
    num = num + 10
  end
  
  local name = "car_getnum"..num
  
  local sql = "update already_get_carnum_inkey set %s=%s +%d; commit;"
	sql = string.format(sql, name, name, add_item_num)
	dblib.execute(sql, function(dt) end)
