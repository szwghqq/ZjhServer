TraceError("init act_longzhou_lib...初始化竞技场信息")
if act_longzhou_lib and act_longzhou_lib.on_after_user_login then
	eventmgr:removeEventListener("h2_on_user_login", act_longzhou_lib.on_after_user_login);
end

if act_longzhou_lib and act_longzhou_lib.ontimecheck then
	eventmgr:removeEventListener("timer_minute", act_longzhou_lib.ontimecheck);
end

if act_longzhou_lib and act_longzhou_lib.ongameover then 
	eventmgr:removeEventListener("game_event", act_longzhou_lib.ongameover);
end
 
--德州专用
if act_longzhou_lib and act_longzhou_lib.ongamebegin then  
	eventmgr:removeEventListener("game_begin_event", act_longzhou_lib.ongamebegin);
end

if not act_longzhou_lib then
	act_longzhou_lib = _S
	{ 
		...
		send_fajiang_msg = NULL_FUNC,		--发送发奖消息 
	 	autoNotifyClientForStart = NULL_FUNC, --自动通知活动时间到了
	 	start_baoming_date = "2012-06-21",	--活动报名开始时间
		statime = "2012-06-21 19:00:00",  --活动开始时间
	    endtime = "2012-06-27 23:00:00",  --活动结束时间 
	    rank_endtime = "2012-06-30 00:00:00",	--排行榜结束时间
	      
	    start_day = 20120621,	--用于计算第几天 开始时间
	 --   exttime = "2011-11-14 00:00:00",  --只能领奖时间
        ...
        STATIC_DB_SORT_ID = -999,
	}
end

--德州专用(传递  桌号)
function act_longzhou_lib.ongamebegin(e)
	local deskno=e.data.deskno
	if(deskno==nil)then return end
	act_longzhou_lib.init_invate_match(deskno)
	
end
 
--检查有效时间
--3 可以报名,2可以比赛和报名,3可以领奖, 0过期
function act_longzhou_lib.check_datetime()
	local statime = timelib.db_to_lua_time(act_longzhou_lib.statime);
	local endtime = timelib.db_to_lua_time(act_longzhou_lib.endtime);
	local lj_time = timelib.db_to_lua_time(act_longzhou_lib.rank_endtime); 
	local sys_time = os.time();
	--可以领奖和增加游戏时间
	if(sys_time >= statime and sys_time <= endtime) then
		    local tdate = os.date("*t", sys_time);		    
	        if (tdate.hour >= 19 and tdate.hour < 23)  then
	            return 2;
			else
				return 3
	        end	        
...
--判断是不是有效的比赛
function act_longzhou_lib.can_join_invite_match(user_info,deskinfo,match_num)
	--TraceError("--判断是不是有效的比赛, userId:"..user_info.userId.." match_num:"..match_num)
	
	--1.时间判断 
	if(act_longzhou_lib.check_datetime()~= 2)then
	...
	--德州
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		--判断时间的合法性,0不合法，1只能填领奖信息，2能填领奖信息和比赛
		--if(deskinfo ~= nil and deskinfo.smallbet ~= act_longzhou_lib.room_smallbet1 and deskinfo.smallbet ~= act_longzhou_lib.room_smallbet2 and deskinfo.smallbet ~= act_longzhou_lib.room_smallbet3 and deskinfo.smallbet ~= act_longzhou_lib.room_smallbet4 and deskinfo.smallbet ~= act_longzhou_lib.room_smallbet5)then
		if(deskinfo == nil or deskinfo.smallbet < act_longzhou_lib.room_smallbet2) then
    ...
    --棋牌
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
	...
	--	local playercount = deskinfo.playercount 
    	if(gamepkg.name == "zysz" and tonumber(groupinfo.groupid) ~= act_longzhou_lib.zysz_gs_id)then
...
--游戏结束采集盘数
function act_longzhou_lib.ongameover(gameeventdata)
	
	--活动时间有效性
	local check_time = act_longzhou_lib.check_datetime()
	...
	--游戏事件验证
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name or act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
    ...
    for k,v in pairs(gameeventdata.data) do
		...
		local match_num = act_longzhou_lib.get_invate_match_count(deskno)		--获得桌子数据
		local match_type = 1
 
		--TraceError("游戏结束采集盘数,match_num:"..match_num.." 用户id："..userid.." 游戏名："..gamepkg.name)
		--判断是否满足采集条件  (德州 职业场 专家场   人手 >= 4) (麻将  不判断  4人都加) (智勇三张  高手场4人及以上)
		if(act_longzhou_lib.can_join_invite_match(user_info,deskinfo,match_num))then
			
			--德州
			if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
				if(deskinfo.smallbet == act_longzhou_lib.room_smallbet1)then		--业余场
					match_type = 1;
				elseif(deskinfo.smallbet >= act_longzhou_lib.room_smallbet2 and deskinfo.smallbet < act_longzhou_lib.room_smallbet3)then		--职业场
					match_type = 2;
					...
					if(user_info.sign_ruslt == "1" or user_info.sign_ruslt == "3")then
						...
						act_longzhou_lib.update_invite_db(user_info,match_gold,match_type, 0)
					end
					 
				elseif(deskinfo.smallbet >= act_longzhou_lib.room_smallbet3)then		--专家场
					match_type = 3;
					--判断用户在职业场还是专家场打牌
    			...
    			if(gamepkg.name == "zysz")then
					...
					if(match_gold > 0)then		--麻将只记录赢家
						...
						if(user_info.qp_sign_ruslt == "1" or user_info.qp_sign_ruslt == "3")then
							...
							if(tonumber(groupinfo.groupid) ~= act_longzhou_lib.mj_yk_id)then	--游客区不进入加分
								if(tonumber(groupinfo.groupid) == act_longzhou_lib.mj_xunlian_id or tonumber(groupinfo.groupid) == act_longzhou_lib.mj_xs_id)then	--练习场和新手区 每胜1局加1分
									match_gold = 1
								elseif(tonumber(groupinfo.groupid) == act_longzhou_lib.mj_gs_id)then		--高手入门每胜1局加2分
									match_gold = 2
								elseif(tonumber(groupinfo.groupid) == act_longzhou_lib.mj_8f_id or tonumber(groupinfo.groupid) == act_longzhou_lib.mj_super_id)then		--8番起胡和超级高手每胜1局加3分
									match_gold = 3
								end
...
function act_longzhou_lib.update_invite_db(user_info,match_gold,match_type, sign)
	...
	if(sign == 0)then
		...
		dblib.execute(sql, nil, act_longzhou_lib.STATIC_DB_SORT_ID);
		act_longzhou_lib.update_play_count(user_info,match_type)
	else
...
--游戏结束采集盘数  更新玩的盘数
act_longzhou_lib.update_play_count=function(user_info,match_type)
...
act_longzhou_lib.autoNotifyClientForStart = function(min)
	local now_time = os.date("*t",os.time());
	local hd_start_time = os.date("*t",timelib.db_to_lua_time(act_longzhou_lib.statime));
	local hd_end_time = os.date("*t",timelib.db_to_lua_time(act_longzhou_lib.endtime));
	...
	if(now_time.year == hd_start_time.year and now_time.month == hd_start_time.month and now_time.day == hd_start_time.day and now_time.hour == hd_start_time.hour and min == hd_start_time.min) then
		...
		for k, v in pairs(userlist) do
	        ...
	        act_longzhou_lib.invite_update_user_play_count(v);
   		end
	end
end
 
--定时刷新事件
act_longzhou_lib.ontimecheck = function(e)
	
	--检查并主动通知客户端活动上线
 	act_longzhou_lib.autoNotifyClientForStart(e.data.min);
 	
  	--10分钟要刷一次 
  	if(act_longzhou_lib.check_datetime() == 0)then	--判断活动时间过期
  		--TraceError("定时刷新事件   活动时间过期")
  		return
  	end
  	
	if(act_longzhou_lib.refresh_invate_time == -1 or os.time() > act_longzhou_lib.refresh_invate_time + 60*1)then
		--TraceError("定时刷新事件  10分钟要刷一次 ");
    	act_longzhou_lib.refresh_invate_time = os.time();
    	act_longzhou_lib.init_invite_ph();
    end
    ...
    if ((nowHour == 20 and nowMin == 0)
		...
		if(act_longzhou_lib.last_msg_time < timelib.db_to_lua_time(tmp_time))then
			--德州
			if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		    	broadcast_by_msgtype("match_msg_noti3",0)
		    	act_longzhou_lib.last_msg_time = os.time();
		    end
	  ...
	  --定时发送前3名发奖消息
	if ((nowHour == 23 and nowMin == 0)
        or (nowHour == 23 and nowMin == 5)
		or (nowHour == 23 and nowMin == 10)
		or (nowHour == 23 and nowMin == 15)
		or (nowHour == 23 and nowMin == 20))then
		
		--发送发奖消息
		act_longzhou_lib.send_fajiang_msg()
	end
end

--初始化排行榜
function act_longzhou_lib.init_invite_ph()
	--TraceError("-->>>>初始化排行榜")
	...
	dblib.execute(sql,function(dt)	
	    ...
	    end, act_longzhou_lib.STATIC_DB_SORT_ID);
    end
    
    --德州
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then	    				
	    --初始化业余场排行
	    init_match_ph(act_longzhou_lib.invite_ph_list_yy, 1)
	    --初始化职业场排行
	    init_match_ph(act_longzhou_lib.invite_ph_list_zy, 2)
	    --初始化专家场排行
	    init_match_ph(act_longzhou_lib.invite_ph_list_zj, 3)
	end
	
	--棋牌
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
    	--act_longzhou_lib.invite_qp_mj_ph_list = {}; --麻将排名
		--act_longzhou_lib.invite_qp_zysz_ph_list = {};	--智勇三张排名
 
		--初始化麻将排行
	    init_match_ph(act_longzhou_lib.invite_qp_mj_ph_list,2)
	    --初始化智勇三张排行
	    init_match_ph(act_longzhou_lib.invite_qp_zysz_ph_list,3)
    end
	
end

--请求竞技场的排行榜
function act_longzhou_lib.on_recv_invite_ph_list(buf)
	...
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		...
		if(query_match_type == 1)then		--1，业余场
			invite_paimin_list = act_longzhou_lib.invite_ph_list_yy
		elseif(	query_match_type == 2) then		--2，职业场
			invite_paimin_list = act_longzhou_lib.invite_ph_list_zy
		elseif(	query_match_type == 3) then		--3，专家场
			invite_paimin_list = act_longzhou_lib.invite_ph_list_zj
		end
		...
		--发送排行榜 
		act_longzhou_lib.send_pm_list(user_info, baoming_sign, my_win_gold, my_mc, my_king_count, my_real_play_count, send_len, invite_paimin_list, map_values)
    ...
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
    	...
    	if(query_match_type == 2)then		--麻将
			invite_paimin_list = act_longzhou_lib.invite_qp_mj_ph_list
		elseif(	query_match_type == 3) then		--智勇三张
			invite_paimin_list = act_longzhou_lib.invite_qp_zysz_ph_list 
		end
...
--发送排行榜
function act_longzhou_lib.send_pm_list(user_info, baoming_sign, my_win_gold, my_mc, my_king_count, my_real_play_count, send_len, invite_paimin_list, map_values)
	--TraceError("发送排行榜")
...
function act_longzhou_lib.on_recv_invite_dj(buf)
...
function act_longzhou_lib.init_invate_match(deskno)
	----TraceError("--生成比赛ID，并记录这次比赛的人数，每个桌子在同一时刻，只会有一场比赛，所以直接用桌子号+时间就是唯一的")
	local deskinfo = desklist[deskno];
	if deskinfo==nil then return -1 end;
	if(deskinfo ~= nil and deskinfo.smallbet < act_longzhou_lib.room_smallbet2 )then
		return -1
	end
	...
	local match_time_status = act_longzhou_lib.check_datetime();
...
--[[
--得到邀请赛的ID
function act_longzhou_lib.get_invate_match_id(deskno)
	----TraceError("--得到邀请赛的ID")
	local deskinfo = desklist[deskno];
	if deskinfo==nil then return -1 end;
...
--获得桌子数据（德州和棋牌   共用）
function act_longzhou_lib.get_invate_match_count(deskno)
...
function act_longzhou_lib.on_recv_refresh_timeinfo(buf)
	--这个方法在这里没有用，所以直接返回 
	local user_info = userlist[getuserid(buf)]; 
	if(user_info == nil)then return end
	local match_time_status = act_longzhou_lib.check_datetime();--有效是1 无效是0
	--有效：距离结束时间1，距离开场时间0
	--无效：距离结束时间0，距离开场时间1
	local flag1=0;
	local flag2=0;
...
function act_longzhou_lib.on_recv_already_know_reward(buf)
...
--用户登录后事件（德州和棋牌  共用）
act_longzhou_lib.on_after_user_login = function(e)
	...
	if(match_time_status == 1 or match_time_status == 3)then 
		...
		act_longzhou_lib.invite_match_fajiang(userinfo)
	elseif(match_time_status == 0)then
		--TraceError("-- 用户登录后事件     match_time_status->"..match_time_status.."  时间无效")
		return
	end
	
	act_longzhou_lib.invite_update_user_play_count(userinfo)
	
end

--用户登录后初始化数据
function act_longzhou_lib.invite_update_user_play_count(user_info)
		...
		if(user_info.yy_play_count == nil) then
			user_info.yy_play_count = 0;
		end
		
		if(user_info.zy_play_count == nil)then
			user_info.zy_play_count = 0;
		end
		
		if(user_info.zj_play_count == nil)then
			user_info.zj_play_count = 0;
		end
		
		--初始化竞技场成绩
		if(user_info.score_2 == nil)then
			user_info.score_2 = 0
		end
		if(user_info.score_3 == nil)then
			user_info.score_3 = 0
		end
...
--计算第几场（德州和棋牌  共用）
function act_longzhou_lib.consider_screen()
	 
	local t1 = act_longzhou_lib.start_day
...
function act_longzhou_lib.invite_match_fajiang(user_info)
	--TraceError("给玩家发奖，用户id："..user_info.userId)
 
	local mc = -1;
 
	local screen_n = act_longzhou_lib.consider_screen()	--计算第几场
	...
	local jutifajiang = function(i,user_info,reward,match_type)
		...
		if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
			...
			--加钱
			usermgr.addgold(user_info.userId,reward,0,g_GoldType.longzhou_match,-1,1);
			--记录发奖日志
			...
			--备份数据库 以备算总排名使用（因为每天数据会清零）
			sql = "INSERT INTO log_t_invite_pm(user_id,nick_name,match_type,win_gold,sys_time)  (SELECT user_id,nick_name,match_type,win_gold,NOW() FROM t_invite_pm );commit;";
			dblib.execute(sql);
		end
    	...
    	if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
    		--给发奖 加藏宝图 
			local add_items_result = act_longzhou_lib.chaxun_add_items(user_info, 8005, reward, 1)
	...
	--发奖
	local fajiang = function(user_info,match_type,reward1,reward2,reward3,reward4)
					...
					for i = 1,len do
						...
						if(dt[i].get_reward_time ~= nil)then
							get_reward_time = timelib.db_to_lua_time(dt[i].get_reward_time) or 0
                        		  	...
                        		  	if(dt[i].user_id == user_info.userId and get_reward_time < timelib.db_to_lua_time(act_longzhou_lib.statime))then
                            if(i == 1)then
            					...
            					sql = string.format(sql,user_info.userId,match_type)
            					dblib.execute(sql, function(dt) end, act_longzhou_lib.STATIC_DB_SORT_ID)
                                break
					    end
					end
				end
	...
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		--[[
		专家场：第一名300w；第二名150w；第三名80w；4-5名30w；
 		 职业场：第一名100w；第二名60w；第三名25w；4-5名10w；
]]
		...
		--发职业场的奖
		fajiang(user_info,2,1000000,600000,250000,100000);
		
		--发专家场的奖
		fajiang(user_info,3,3000000,1500000,800000,300000);
	end
    ...
    --[[
    	...
    	--发麻将的奖 
    	if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == "mj")then
			fajiang(user_info,2,100,50,20,10,5,2);
		end
	 
		--发智勇三张的奖
		if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == "zysz")then
...
--请求活动时间状态（德州和棋牌   共用）
function act_longzhou_lib.on_recv_activity_stat(buf)

	local user_info = userlist[getuserid(buf)]; 
	if(user_info == nil)then return end
	
	--0，活动无效，不显示相关UI；2，活动有效，比赛阶段
	local check_stat = act_longzhou_lib.check_datetime()
	
	local endtime = timelib.db_to_lua_time(act_longzhou_lib.endtime);
	local ranktime =  timelib.db_to_lua_time(act_longzhou_lib.rank_endtime);
	local sys_time = os.time();
...
--请求报名比赛
function act_longzhou_lib.on_recv_sign(buf)
	...
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		...
		if(sign == 2)then	--2，职业场
			...
			elseif(shiptickets_count < 3)then	--判断报名资格
				sign_ruslt = 2
		 		--TraceError("请求报名比赛   职业场   门票不足     2   用户id："..user_info.userId.." 游戏名："..gamepkg.name)
			
			else--报名成功，需扣除资格证数量
				sign_ruslt = 1
				act_longzhou_lib.sign_succes(user_info, 3, sign)
			end
		...
		elseif(sign == 3)then	--3，专家场
			...
			elseif(shiptickets_count < 5)then	--判断报名资格
				...
				sign_ruslt = 1
				act_longzhou_lib.sign_succes(user_info, 5, sign)
			end
    ...
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
    	--查询门票数量
		local shiptickets_count = user_info.bag_items[act_longzhou_lib.qp_mp_id] or 0;
	    
		if(sign == 2)then	--2，麻将
			if(act_longzhou_lib.check_datetime() == 0)then	--判断活动有效性
				sign_ruslt = 3
			...
			elseif(shiptickets_count < 2)then	--判断报名资格
				...
				sign_ruslt = 1
				act_longzhou_lib.sign_succes(user_info, 2, sign)
			end
		...
		elseif(sign == 3)then	--3，智勇三张
			...
			elseif(shiptickets_count < 10)then	--判断报名资格
				sign_ruslt = 2
				--TraceError("请求报名比赛    智勇三张    门票不足      用户id："..user_info.userId.." 游戏名："..gamepkg.name)
			
			else--报名成功，需扣除资格证数量
				sign_ruslt = 1
				act_longzhou_lib.sign_succes(user_info, 10, sign)
			end
...
--报名成功
function  act_longzhou_lib.sign_succes(user_info, k_count, match_type)
    ...
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
		...
		--加背包
		local add_items_result = act_longzhou_lib.chaxun_add_items(user_info, act_longzhou_lib.qp_mp_id, -k_count, 1)
		 
		--TraceError("--报名成功，需扣除门票数量："..k_count.." 游戏名："..gamepkg.name)
    end
	 
	--报名写数据库
	act_longzhou_lib.inster_invite_db(user_info,0,match_type,xie_sign)
...
--报名写数据库
function act_longzhou_lib.inster_invite_db(user_info,match_gold,match_type, xie_sign)
...
--请求购买比赛券
function act_longzhou_lib.on_recv_buy_ticket(buf)
	...
	if(vip_level<3)then
		act_longzhou_lib.send_buy_ticket_result(user_info, 5)
	    return 
	end
	]]--
	--TraceError("请求购买比赛券     用户id："..user_info.userId.." 游戏名："..gamepkg.name)
	
	local gold = user_info.gamescore		--获得用户筹码
	local ruslt = 0
	...
	if(check_time == 0 or check_time==1 or sys_time > endtime)then
		...
		--发送购买比赛券结果
		act_longzhou_lib.send_buy_ticket_result(user_info, ruslt)
		return
	end
	...
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		...
		elseif(gold < act_longzhou_lib.OP_MENPIAO_PRIZE["tex"])then	--判断筹码小于2万筹码
			ruslt = 0
			...
			ruslt = 1
	 		--减2万筹码
		    usermgr.addgold(user_info.userId, -1*act_longzhou_lib.OP_MENPIAO_PRIZE["tex"], 0, g_GoldType.baoxiang, -1);
    ...
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
    	--查询道具情况
		local chaxun_items_result = act_longzhou_lib.chaxun_add_items(user_info, act_longzhou_lib.qp_mp_id, 0, 0)
		...
		elseif(gold < act_longzhou_lib.OP_MENPIAO_PRIZE["qp"])then	--判断筹码小于10万金币
			ruslt = 0
		...
		elseif(desk_no ~= nil)then	--判断是否在牌桌内
			...
			return
 			...
 			--减10万金币
		    usermgr.addgold(user_info.userId, -1*act_longzhou_lib.OP_MENPIAO_PRIZE["qp"], 0, tSqlTemplete.goldType.HD_NEW_YEAR, -1);
		    
			--加背包门票
			local add_items_result = act_longzhou_lib.chaxun_add_items(user_info, act_longzhou_lib.qp_mp_id, 1, 1)
...
--发送购买比赛券结果
function act_longzhou_lib.send_buy_ticket_result(user_info, ruslt)
...
function act_longzhou_lib.chaxun_add_items(user_info, item_id, item_num, use_type)
...
-- 全服发广播(棋牌专用)
function act_longzhou_lib.ontimer_broad(tips,flag)
    --如果提示不为nil 全服发广播。
...
--发送发奖消息
act_longzhou_lib.send_fajiang_msg = function()
	...
	if(act_longzhou_lib.cfg_tex_game_name[gamepkg.name] == gamepkg.name)then
		...
		dblib.execute(sql,function(dt)	
				if(dt ~= nil and  #dt > 0)then
					...
					for i = 1,len do
						if(i == 1)then
							msg = _U(tex_lan.get_msg(user_info, "match_msg_awards_1"))..dt[i].nick_name.._U(tex_lan.get_msg(user_info, "match_msg_lz_awards_type_2"));
		...
		dblib.execute(sql,function(dt)	
				if(dt ~= nil and  #dt > 0)then
					...
					for i = 1,len do
						if(i == 1)then
							msg = _U(tex_lan.get_msg(user_info, "match_msg_awards_1"))..dt[i].nick_name.._U(tex_lan.get_msg(user_info, "match_msg_lz_awards_type_3"));
    ...
    if(act_longzhou_lib.cfg_qp_game_name[gamepkg.name] == gamepkg.name)then
		...
		dblib.execute(sql,function(dt)	
				if(dt ~= nil and  #dt > 0)then
					...
					act_longzhou_lib.ontimer_broad(msg,1);
		...
		dblib.execute(sql,function(dt)	
				if(dt ~= nil and  #dt > 0)then
					...
					act_longzhou_lib.ontimer_broad(msg1,1);
...
{ 
    --竞技场相关协议
    ["INVITEPHLIST"] = act_longzhou_lib.on_recv_invite_ph_list,  --请求邀请赛的排行榜
   -- ["INVITEDJ"] = act_longzhou_lib.on_recv_invite_dj,  --填写邀请赛的领奖结果
    ["INVITEBTN"] = act_longzhou_lib.on_recv_refresh_timeinfo, --请求刷新图标按钮信息
    ["INVITEGIF"] = act_longzhou_lib.on_recv_already_know_reward, --客户端通知已经点过领奖按钮了
    
    ["INVITEPHDATE"] = act_longzhou_lib.on_recv_activity_stat, --请求活动时间状态
    ["INVITESIGNUP"] = act_longzhou_lib.on_recv_sign, --请求报名比赛
    ["INVITEBUYTK"] = act_longzhou_lib.on_recv_buy_ticket, --请求购买比赛券
}
...
eventmgr:addEventListener("h2_on_user_login", act_longzhou_lib.on_after_user_login);
eventmgr:addEventListener("timer_minute", act_longzhou_lib.ontimecheck); 
eventmgr:addEventListener("game_event", act_longzhou_lib.ongameover); 
eventmgr:addEventListener("game_begin_event", act_longzhou_lib.ongamebegin);  
